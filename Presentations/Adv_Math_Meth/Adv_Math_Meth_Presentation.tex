...
\end{frame}
%
\begin{frame}{Some Motivation 2}
	\begin{itemize}
		\item States of elementary particles such as the electron are described by a state space $\mathcal{H} =\mathbb{C}^2$.
		\item We want to study rotations acting on this space, and this is done via: the two dimensional irreducible representation of $SU(2)=Spin(3)$
	\end{itemize}
\end{frame}
...
\end{frame}
%
\begin{frame}{Step Back: Axioms of QM}
	\begin{itemize}
		\item A1: The state of a QMS is given by a non-zero vector in a complex vector space $\mathcal{H}$ with Hermitian inner product.
		\item A2: The observables of a QMS are given by self-adjoint linear operators on $\mathcal{H}$
		\item A3: There is a distinguished quantum observable, the Hamiltonian $H$. Time evolution of states $| \psi(t)\rangle\in\mathcal{H}$ is given by the Schrödinger equation 
		\begin{displaymath}
...
\end{frame}
%
\begin{frame}{With Schrödinger Equation}
...
\begin{frame}{Heisenberg Picture}
	\begin{itemize}
		\item Time evolution operator can be used to make a unitary transformation that puts the time-dependence on the observables. This is:
		\begin{displaymath}
  			|\psi(t)\rangle \rightarrow |\psi(t)\rangle_{H}=U^{-1}(t)|\psi(t)\rangle,
		\end{displaymath}
		\begin{displaymath}
  			\mathcal{O}\rightarrow\mathcal{O}_{H}(t)=U^{-1}(t)\mathcal{O}U(t)
		\end{displaymath}
		\item The dynamics is given by a differential equation on the operators
		\begin{displaymath}
  			\frac{d}{dt}\mathcal{O}_{H}(t) = i[H,\mathcal{O}_{H}(t)] 
		\end{displaymath}
	\end{itemize}
\end{frame}
%
\begin{frame}{Rotation of the Spin Vector}
	\begin{itemize}
		\item Putting all together:
		\begin{displaymath}
  			\frac{d}{dt}\mathbf{S}_{H}(t)= i[H,\mathbf{S}_{H}(t)] 
		\end{displaymath}
		the solution will be 
		\begin{displaymath}
  			\mathbf{S}_{H}(t) = U(t)\mathbf{S}_H(0)U(t)^{-1}.
		\end{displaymath}
		\item The spin vector observable evolves in the Heisenberg picture by rotating about the magnetic field vector $\mathbf{B}$ with angular velocity $(ge/2mc)|\mathbf{B}|$
		\end{itemize}
\end{frame}
%
\begin{frame}{Schrödinger and Heisenberg Pictures}

\begin{minipage}[t]{1\columnwidth}%
\begin{center}
\begin{tabular}{|c|c|c|}
\hline 
 & Heisenberg Picture & Schrödinger Picture\tabularnewline
\hline 
\hline 
