...
\end{problem} I did the whole problem in Mathematica and I append the notebook.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\textbf{(iv)} Now, let's compute the equations of motion from the Euler-Lagrange equations. In this part, the Lagrangian is given by
\begin{equation}
  \mathcal{L}_i = -m_i \sqrt{\eta_{\mu\nu}\dot{x}^{\mu}_i\dot{x}^{\nu}_i} - q_iA_{\mu}\dot{x}^{\mu}_i,
\end{equation}
where \begin{equation}
   \dot{x}^{\mu}_i = \frac{dx^{\mu}_i}{d\tau}
\end{equation}
and we know the Euler-Lagrange equation is given by
\begin{equation}
  \frac{d}{d\tau}\left( \frac{\partial\mathcal{L}_i}{\partial\dot{x}^{\alpha}_i}\right) - \frac{\partial\mathcal{L}}{\partial x^{\alpha}_i} = 0.
\end{equation}
For the second term, we have
\begin{equation}
  \frac{\partial\mathcal{L}}{\partial x^{\alpha}_i} = -q_i \dot{x}^{\mu}_i\partial_\alpha A_\mu, 
\end{equation}
now, for the first term we need to use the chain rule, which gives us
\begin{equation}
  \frac{\partial\mathcal{L}_i}{\partial\dot{x}^{\alpha}_i} = -m_i\frac{\eta_{\alpha\nu}\dot{x}^\nu_i}{\sqrt{\eta_{\mu\nu}\dot{x}^{\mu}_i\dot{x}^{\nu}_i}} - q_i A_\alpha,
\end{equation}
but we know that
\begin{equation}
  \sqrt{\eta_{\mu\nu}\dot{x}^{\mu}_i\dot{x}^{\nu}_i} = 1,
...
thus, we have
\begin{equation}
  \frac{\partial\mathcal{L}_i}{\partial\dot{x}^{\alpha}_i} = -m_i\eta_{\alpha\nu}\dot{x}^\nu_i - q_i A_\alpha\implies \frac{d}{d\tau}\frac{\partial\mathcal{L}_i}{\partial\dot{x}^{\alpha}_i} = -m_i\eta_{\alpha\nu}\ddot{x}^\nu_i - q_i\frac{d A_\alpha}{d\tau},
\end{equation}
but 
\begin{equation}
  \frac{d A_\alpha}{d\tau} = \frac{\partial A_\alpha}{\partial x^{\mu}_i}\frac{dx^{\mu}_i}{d\tau} = \pd_\mu A_\alpha \dot{x}^{\mu}_i, 
\end{equation}
which implies that
\begin{equation}
  \frac{d}{d\tau}\frac{\pd\mathcal{L}_i}{\pd\dot{x}^{\alpha}_i} = -m_i\eta_{\alpha\nu}\ddot{x}^\nu_i - q_i\pd_\mu A_\alpha \dot{x}^{\mu}_i \implies   \frac{d}{d\tau}\frac{\partial\mathcal{L}_i}{\partial\dot{x}^{\alpha}_i} = -m_i\ddot{x}_{i\alpha} - q_i\pd_\mu A_\alpha \dot{x}^{\mu}_i
\end{equation}
then, if we combine both terms in the Euler-Lagrange equations, we have
\begin{equation}
  -m_i\ddot{x}_{i\alpha} - q_i\pd_\mu A_\alpha \dot{x}^{\mu}_i + q_i \dot{x}^{\mu}_i\partial_\alpha A_\mu = 0,
\end{equation}
which implies that
\begin{equation}
  m_i\ddot{x}_{i\alpha} = q_i\dot{x}^{\mu}_i(\pd_\alpha A_\mu - \pd_\mu A_\alpha),
\end{equation}
and from this we have
\begin{equation}
  m_i\ddot{x}_{i\alpha} = q_i\dot{x}^{\mu}_i F_{\alpha\mu},
\end{equation}
which can be written as
\begin{equation}
  m_i\eta^{\sigma\alpha}\ddot{x}_{i\alpha} = q_i\eta^{\sigma\alpha}\dot{x}^{\mu}_i F_{\alpha_\mu} \implies m_i \ddot{x}^{\sigma}_{i} = q_i F^{\sigma}_{\mu}\dot{x}^{\mu}_i,
\end{equation}
and if we change $\sigma\rightarrow\alpha$, we have
\begin{equation}
  m_i \ddot{x}^{\alpha}_{i} = q_i F^{\alpha}_{\mu}\dot{x}^{\mu}_i,
\end{equation}
just as we wanted.

\textbf{(v)} Finally, if we set $\alpha=j$ in the previous expression, by the chain rule, we have
\begin{equation}
  m_i \frac{d\tau}{dt}\frac{d}{d\tau}(\frac{dx^j_i}{d\tau}) = q_i F^{j}_\mu \frac{d\tau}{dt}\frac{dx^{\mu}_i}{d\tau},
\end{equation}
which implies that
\begin{equation}
  \frac{d}{dt}\left(m_i \frac{dx^j_i}{d\tau}\right) = q_i F^{j}_\mu \frac{dx^{\mu}_i}{dt} \quad\implies   \frac{d}{dt}\left(\mathbf{p}_i\right) = q_i F^{j}_\mu \frac{dx^{\mu}_i}{dt},
\end{equation}
on the other hand, we know the matrix representation of the Faraday tensor, which is
\begin{equation}
...
\begin{equation}
  F^{\alpha}_\mu = \eta_{\mu\beta} F^{\alpha\beta},
\end{equation}
or in matrix notation
\begin{equation}
  F^{\alpha}_\mu = \begin{bmatrix}
 0 & -E_x & -E_y & -E_z \\
 -E_x & 0 & B_z & -B_y \\
 -E_y & -B_z & 0 & B_x \\
 -E_z & B_y & -B_x & 0  	
  \end{bmatrix}
\end{equation}
but since we're interested in the spatial index in $\alpha$ we have
\begin{equation}
  F^i_\mu\frac{dx^{\mu}_i}{dt} = 
  	\begin{bmatrix}			     
			 -E_x & 0 & B_z & -B_y \\
 			 -E_y & -B_z & 0 & B_x \\
			 -E_z & B_y & -B_x & 0  	
  	\end{bmatrix}\cdot
  	 \begin{bmatrix}
  			c dt/dt \\ -dx_i/dt \\ -dy_i/dt \\ -dz+i/dt
  	 \end{bmatrix}
\end{equation}
then we have
\begin{equation}
  \frac{d\mathbf{p}_i}{dt} = q_i 
  \begin{bmatrix}
  	E_x - B_z dy_i/dt + B_y dz_i/dt \\
  	E_y + B_zdx_i/dt - B_x dz_i/dt \\	
  	E_z - B_ydx_i/dt + B_x dy_i/dt
  \end{bmatrix},
\end{equation}
...
\end{equation}
which is the equation of motion for particle i in the presence of an electric and magnetic field, the RHS of the previous equation is the Lorentz force.
\pagebreak
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
